...
def print_tree(tree, indent='',level=0):
    ...
    if tree.results:
        ...
        print(indent + 'T-> ', end='')
        print_tree(tree.tb, indent + '  ',level)
        print(indent + 'F-> ', end='')
        print_tree(tree.fb, indent + '  ',level)
...
if __name__ == '__main__':
    ...
    testCase = [att1,att2,att3,att4,type]

    test1 = []
    test2 = []

    for i in range(0,int(len(trainingData)/2)):
        test1.append(trainingData[i])
    for i in range(int(len(trainingData)/2),int(len(trainingData))):
        test2.append(trainingData[i])

    tree1 = build_tree(test1)
    tree2 = build_tree(test2)

    k1 = classify(testCase,tree1)
    k2 = classify(testCase,tree2)
