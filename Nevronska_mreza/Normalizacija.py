...
def minMaxNrom(training_data, testing_data):
    ...
    maxs=[-float("inf"),-float("inf"),-float("inf"),-float("inf")]
    ...
    test_maxs=[-float("inf"),-float("inf"),-float("inf"),-float("inf")]
    test_mins=[float("inf"),float("inf"),float("inf"),float("inf")]
    for row in testing_data:
        for i in range(4):
            if row[i]>test_maxs[i]:
                test_maxs[i]=row[i]
            if row[i]<test_mins[i]:
                test_mins[i]=row[i]

    for row in testing_data:
        for i in range(int(len(row)-1)):
            row[i]=(row[i]-test_mins[i])/(test_maxs[i]-test_mins[i])
