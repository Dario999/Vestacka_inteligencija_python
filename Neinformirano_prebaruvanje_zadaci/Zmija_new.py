...
def coalision(zmija,glava):
    lista = list(zmija)
    if lista.__contains__(glava):
        return False
    else:
        return True

def out_of_range(head):
    x = glava[0]
    y = glava[1]
    if x > 9 or x < 0:
        return False
...
def prodolziPravo(state):
    ...
    if out_of_range(new_glava) and coalision(zmija,new_glava):
        if ateApple(new_glava,crveni_jabolki):
            ...
            lista = list(zmija)
            lista.append(new_glava)
            list_apples = list(zeleni_jabolki)
            list_apples.remove(new_glava)
            return (nasoka,tuple(lista),tuple(list_apples),tuple(crveni_jabolki))
        else:
            lista = list(zmija)
            lista.append(new_glava)
            lista.pop(0)
            return (nasoka,tuple(lista),tuple(zeleni_jabolki),tuple(crveni_jabolki))
...
if __name__ == '__main__':
    ...
    Initial = ('D',zmija,tuple(zeleni_jabolki),tuple(crveni_jabolki))
    snake = Snake(Initial)
