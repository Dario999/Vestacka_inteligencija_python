    ...
    def successor(self, state):
        ...
        for i in range(0,n):
            for j in range(0,n):
                ...
                if i > 0:
                    new_state[i-1][j] = 0 if new_state[i-1][j] == 1 else 1
                if j > 0:
                    new_state[i][j-1] = 0 if new_state[i][j-1] == 1 else 1
                if i < n-1:
                    new_state[i+1][j] = 0 if new_state[i+1][j] == 1 else 1
                if j < n-1:
                    new_state[i][j+1] = 0 if new_state[i][j+1] == 1 else 1
                ...
                successors['x: '+ str(i) + ', y: ' + str(j)] = new_state
...
if __name__ == '__main__':
    ...
    redici = tuple(map(tuple, [polinja[i*n:(i+1)*n] for i in range(0, n)]))
    print(redici)

    crnobelo = crnoBelo(redici,None)
