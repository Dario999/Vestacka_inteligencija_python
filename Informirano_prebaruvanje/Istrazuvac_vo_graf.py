...
def proveriRebro(player,player_new,links):
    if (player,player_new) in links:
        return False
    elif (player_new,player) in links:
        return False
    else:
        return True

def isTaken(state):
    player,z1,z2,links = state
    if player == z1:
        return player,0,z2,links
    elif player == z2:
        return player,z1,0,links
    else:
        return state
...
def Gore(state):
    player,z1,z2,links = state
    if (player > 4 and player != 9 and player != 12):
        player_new = player - 4
        if proveriRebro(player,player_new,links):
            lista = list(links)
            lista.append((player,player_new))
            links = tuple(lista)
            state1 = player_new,z1,z2,links
            new_state = isTaken(state1)
            if new_state != state:
                return new_state
            else:
                return player_new,z1,z2,links
        else:
            return state
    else:
        return state
...
def Dolu(state):
    player, z1, z2,links = state
    if (player < 13 and player != 5 and player != 8):
        player_new = player + 4
        if proveriRebro(player,player_new,links):
            lista = list(links)
            lista.append((player, player_new))
...
def Levo(state):
    player, z1, z2 ,links = state
    if (player%2 == 0 or player == 7 or player == 11):
        player_new = player - 1
        if proveriRebro(player,player_new,links):
            lista = list(links)
            lista.append((player, player_new))
...
def Desno(state):
    player, z1, z2, links = state
    if (player % 2 == 1 or player == 6 or player == 10):
        player_new = player + 1
        if proveriRebro(player, player_new,links):
            lista = list(links)
            lista.append((player, player_new))
...
class Zvezdi(Problem):
    ...
    def successor(self, state):
        ...
        #Gore
        state_new = Gore(state)
        if state_new != state:
            successors['Gore'] = state_new

        #Dolu
        state_new = Dolu(state)
        if state_new != state:
            successors['Dolu'] = state_new

        #Levo
        state_new = Levo(state)
        if state_new != state:
            successors['Levo'] = state_new

        #Desno
        state_new = Desno(state)
        if state_new != state:
            successors['Desno'] = state_new

        #DoluDesno
        state_new = DoluDesno(state)
        if state_new != state:
            successors['DoluDesno'] = state_new

        #GoreLevo
        state_new = GoreLevo(state)
        if state_new != state:
            successors['GoreLevo'] = state_new
...
if __name__ == '__main__':
    ...
    Initial = (IgracPozicija,Z1Pozicija,Z2Pozicija,links)

    problem = Zvezdi(Initial)


    answer = breadth_first_graph_search(problem)

    print(answer.soluton())
